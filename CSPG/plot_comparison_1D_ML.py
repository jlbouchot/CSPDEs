...
def plot_1D(infile, outfile, L_to_plot, v_to_plot, k_to_plot, which_to_plot = None):
    ...
    spde_model.refine_mesh(ratio=2**(np.max(L_to_plot)+2)) # Refine to 2 levels larger than the largest level we consider. !!!!!! To rechange back once the first tests work
